...
warn "\n\n> microTSS-CAGE is starting with a job id $result_directory_id.\n\n".
	"\tInput info:\n".
...
if( scalar(@absolute_cage_files) == 1 ){
	...
	###----------------------START--------- here put the CAGE cluster
	##$working_directory/CAGE_tag_cluster/tc_centile/
	##$absolute_cage_files[0]
	system "mkdir $working_directory/CAGE_tag_cluster/tc_centile" unless -d "$working_directory/CAGE_tag_cluster/tc_centile";
	system "cat $absolute_cage_files[0] > $working_directory/CAGE_tag_cluster/tc_centile/all_chr_tc_centile.bed";
	system "python $FindBin::Bin/src/get_representatives.py $working_directory/CAGE_tag_cluster/tc_centile/all_chr_tc_centile.bed $working_directory/CAGE_tag_cluster/tc_centile/all_chr_tc_centile_representative.bed $working_directory/CAGE_tag_cluster/tc_centile/all_chr_tc_centile_newClusterNames.bed";

	###---------------------END------------ here put the CAGE cluster
	...
	warn "\t\tFinished in ".((time()-$time)/60)." minutes.\n";
	warn "\tEnd Getting the sequence around CAGE tag cluster representatives.\n";
}
else{
